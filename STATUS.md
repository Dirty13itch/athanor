...
| 2: RTX 4090 (ASUS) | devstral-small-2 | ~16/24.6 GB | vllm-coder | 8006 |
...
Other containers: `athanor-agents` (9000), `athanor-gpu-orchestrator`, `alloy`, `wyoming-whisper` (10300), `speaches` (8200), `dcgm-exporter` (9400), `node-exporter`

### WORKSHOP (.225) â€” 9 containers

| GPU | Model | VRAM | Temp | Container | Port |
|-----|-------|------|------|-----------|------|
| 0: RTX 5090 | Qwen3.5-35B-A3B-AWQ-4bit | 27.0/32.6 GB | 37°C | vllm-node2 | 8010 |
| 1: RTX 5060 Ti | ComfyUI | 1.1/16.3 GB | 34°C | comfyui | 8188 |

Other: `athanor-dashboard` (3001), `athanor-eoq` (3002), `athanor-ulrich-energy` (3003), `athanor-ws-pty-bridge` (3100), `open-webui` (3000), `aesthetic-scorer`, `alloy`, `dcgm-exporter`, `node-exporter`

**Note:** Ollama was disabled (systemd `disable`) — was squatting on both GPUs, blocking vLLM. Restored in Session 58.
...
| foundry:8006 | devstral-small-2 | ✅ Healthy (model changed from Qwen3-Coder) |
| foundry:9000 | Agent Server (9 agents) | ✅ Healthy |
| foundry:9200 | GPU Orchestrator | ✅ Healthy (fixed Redis auth Session 58) |
| workshop:8010 | Qwen3.5-35B-A3B-AWQ-4bit | ✅ Restored (Ollama displaced, fixed Session 58) |
| workshop:3001 | Dashboard | ✅ Healthy |
| workshop:3002 | EoBQ | ✅ Healthy |
| workshop:3003 | Ulrich Energy | ✅ Healthy |
| dev:8001 | Qwen3-Embedding-0.6B | ✅ Healthy |
| dev:8003 | Qwen3-Reranker-0.6B | ✅ Healthy |
| vault:4000 | LiteLLM | ✅ Healthy (auth-gated) |
...
## Repo and Host Convergence

Canonical parity artifact: `docs/atlas/inventory/completion/environment-parity-census.json`

| Surface | Status | Details |
|---------|--------|---------|
| DESK checkout | Branch `codex/full-tree-sync-20260321` | Clean tree, 9 ahead / 365 behind origin/main |
| GitHub main | Diverged | 365 commits on main not in current branch (Codex, drift-check, credential rotation, SoulForge, etc.) |
| DESK WSL | Partial | Default Ubuntu distro is present but currently stopped |
| DEV adapter host | Partial | `codex`, `gemini`, `aider`, and `goose` are available; `claude` and `kimi` binaries exist under `~/.local/bin` but PATH is not normalized; CCR config is missing |
| LiteLLM direct catalog | Auth-gated | Direct `/v1/models` returns `401` without key; audit context can read the catalog with auth |

Release truth rule: reviewed `GitHub main` plus matching deployed runtime plus green audit evidence is shipped. Dirty DESK state is not shipped.

## Warnings / Remediation Backlog
...
| **n8n workflow activation** | Signal pipeline idle | Shaun needs to activate the workflow in the VAULT UI |
| **Home Assistant depth decisions** | Home-agent depth capped at current HA graph | Shaun needs to finish Lutron/UniFi and automation scope |
| **Kindred decision** | Portfolio lane remains deferred | Shaun needs to make a go/no-go call |

### Stabilization Backlog

- Dashboard E2E, unit, and build gates are green.
- Completion audit is ready with zero failed jobs.
- Remaining backlog items are external dependency packets and deferred promotion decisions, not platform blockers.

### External Dependency Packets

Canonical packet register: [docs/DEPENDENCY-PACKETS.md](./docs/DEPENDENCY-PACKETS.md)
...
### Completed This Session
- **Workshop vLLM worker restored (P0)** — Ollama (systemd service) had displaced vLLM from both Workshop GPUs (29.7 GiB on 5090, 12.75 GiB on 5060 Ti). Stopped Ollama, disabled from boot, restarted vLLM. Qwen3.5-35B-A3B-AWQ-4bit serving on :8010. This restored 5 LiteLLM routes (`worker`, `fast`, `creative`, `utility`, `uncensored`).
- **GPU orchestrator Redis auth fixed (P1)** — Container was crash-looping with `AuthenticationError: Authentication required`. Redis on VAULT now requires a password. Updated compose with auth URL, fixed stale Workshop port (`:8000` → `:8010`). Orchestrator healthy at :9200.
- **Full service mesh verified** — All 10 endpoints green. Notable: FOUNDRY coder model changed to `devstral-small-2` (was `Qwen3-Coder-30B-A3B-Instruct-AWQ`).
- **Git convergence assessed** — Branch `codex/full-tree-sync-20260321` is 9 ahead / 365 behind `origin/main`. Large divergence from Codex and other agent sessions. Needs deliberate merge strategy from Shaun.
- **Ansible GPU orchestrator role updated** — Redis URL now includes password from `athanor_redis_password` var.

### Issues Found & Resolved
| Issue | Root Cause | Fix |
|-------|-----------|-----|
| Workshop vLLM down | Ollama systemd service consuming both GPUs | Stopped + disabled Ollama, restarted vLLM |
| GPU orchestrator crash loop | Redis auth required, not configured | Added password to Redis URL |
| Workshop port mismatch | Compose had `:8000`, should be `:8010` | Fixed in compose |

### Next Actions
1. **Shaun**: Decide merge strategy for `codex/full-tree-sync-20260321` vs `origin/main` (365 commits behind)
2. **Shaun**: Provide pending credentials (NordVPN, Anthropic API key, Google Drive OAuth)
3. Monitor Workshop vLLM stability — Ollama may need to be uninstalled if Open WebUI still auto-triggers it
4. Investigate FOUNDRY coder model change (devstral-small-2 vs Qwen3-Coder)
5. Run fresh completion audit after merge convergence
...
- **All 4 vLLM containers healthy** â€” coordinator:8000 âœ…, utility:8002 âœ…, workshop:8010 âœ…
...
*Last updated: 2026-03-23 22:35 CDT
