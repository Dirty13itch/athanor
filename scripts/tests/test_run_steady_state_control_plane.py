...
def test_build_commands_runs_fixed_point_order_with_steady_state_writer():
    ...
    assert commands[4][1:] == ['scripts/collect_capacity_telemetry.py']
    assert commands[5][1:] == ['scripts/write_quota_truth_snapshot.py']
    assert commands[6][1:] == ['scripts/run_ralph_loop_pass.py', '--skip-refresh']
    ...
    assert commands[-3][1:] == ['scripts/triage_publication_tranche.py', '--write', 'docs/operations/PUBLICATION-TRIAGE-REPORT.md']
    assert commands[-2][1:] == ['scripts/generate_publication_deferred_family_queue.py']
