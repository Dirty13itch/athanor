...
if [ -f /home/shaun/.secrets/litellm-master-key ]; then
    LITELLM_KEY="$(tr -d '\n' < /home/shaun/.secrets/litellm-master-key)"
fi
...
contract "Memory /health returns status=ok" \
    'curl -sf --max-time 5 http://192.168.1.189:8720/health | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if d.get("status")=="ok" else 1)"'

# CONTRACT: Memory health response must include service=memory
contract "Memory /health includes service=memory" \
    'curl -sf --max-time 5 http://192.168.1.189:8720/health | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if d.get("service")=="memory" else 1)"'

# CONTRACT: Memory version field must exist and be non-empty
contract "Memory /health has version field" \
    'curl -sf --max-time 5 http://192.168.1.189:8720/health | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if d.get("version") else 1)"'
...
contract "Memory /v1/memory/stats returns all 6 tiers" \
    'curl -sf --max-time 5 http://192.168.1.189:8720/v1/memory/stats | python3 -c "
import sys,json
d=json.load(sys.stdin)
required={"working","episodic","semantic","procedural","resource","vault"}
...
contract "Memory /v1/memory/store endpoint exists" \
    'CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 -X POST http://192.168.1.189:8720/v1/memory/store -H "Content-Type: application/json" -d "{}"); [ "$CODE" != "404" ] && [ "$CODE" != "000" ]'

# CONTRACT: /v1/search endpoint exists
contract "Memory /v1/search endpoint exists" \
    'CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 -X POST http://192.168.1.189:8720/v1/search -H "Content-Type: application/json" -d "{}"); [ "$CODE" != "404" ] && [ "$CODE" != "000" ]'

$QUIET || echo ""
$QUIET || echo "--- Gateway API (DEV:8700) ---"

# CONTRACT: Gateway health response has status=ok
contract "Gateway /health returns status=ok" \
    'curl -sf --max-time 5 http://192.168.1.189:8700/health | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if d.get("status")=="ok" else 1)"'

# CONTRACT: Gateway health includes service=gateway
contract "Gateway /health includes service=gateway" \
    'curl -sf --max-time 5 http://192.168.1.189:8700/health | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if d.get("service")=="gateway" else 1)"'

# CONTRACT: Gateway version field present
contract "Gateway /health has version field" \
    'curl -sf --max-time 5 http://192.168.1.189:8700/health | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if d.get("version") else 1)"'

# CONTRACT: Gateway /v1/models endpoint exists and returns JSON
contract "Gateway /v1/models returns JSON" \
    'curl -sf --max-time 5 http://192.168.1.189:8700/v1/models | python3 -c "import sys,json; json.load(sys.stdin); sys.exit(0)"'

# CONTRACT: Gateway OpenAPI spec is reachable
contract "Gateway /openapi.json is reachable" \
    'curl -sf --max-time 5 http://192.168.1.189:8700/openapi.json | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if "paths" in d else 1)"'
...
contract "Agent server /health returns status=healthy" \
    'curl -sf --max-time 10 http://192.168.1.244:9000/health | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if d.get("status")=="healthy" else 1)"'

# CONTRACT: Agent count must equal 9
contract "Agent server reports exactly 9 agents" \
    'curl -sf --max-time 10 http://192.168.1.244:9000/health | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if d.get("agent_count")==9 else 1)"'

# CONTRACT: All 9 expected agents are present by name
contract "Agent server has all 9 expected agent names" \
    'curl -sf --max-time 10 http://192.168.1.244:9000/health | python3 -c "
import sys,json
d=json.load(sys.stdin)
...
contract "Agent server /openapi.json is reachable" \
    'curl -sf --max-time 10 http://192.168.1.244:9000/openapi.json | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if "paths" in d else 1)"'
...
contract "Agent server /v1/chat/completions endpoint exists" \
    'CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 -X POST http://192.168.1.244:9000/v1/chat/completions -H "Content-Type: application/json" -d "{}"); [ "$CODE" != "404" ] && [ "$CODE" != "000" ]'

$QUIET || echo ""
$QUIET || echo "--- LiteLLM (VAULT:4000) ---"

# CONTRACT: LiteLLM responds to health check with auth
contract "LiteLLM /health reachable with master key" \
    'curl -sf --max-time 5 -H "Authorization: Bearer ${LITELLM_KEY}" http://192.168.1.203:4000/health >/dev/null'

# CONTRACT: LiteLLM /v1/models returns a list with known aliases
contract "LiteLLM has reasoning model alias" \
    'curl -sf --max-time 5 -H "Authorization: Bearer ${LITELLM_KEY}" http://192.168.1.203:4000/v1/models | python3 -c "
import sys,json
d=json.load(sys.stdin)
...
# CONTRACT: LiteLLM /metrics/ endpoint responds (requires trailing slash)
contract "LiteLLM /metrics/ endpoint responds" \
    'curl -sf --max-time 5 -H "Authorization: Bearer ${LITELLM_KEY}" http://192.168.1.203:4000/metrics/ | grep -q "litellm"'
...
contract "Qdrant /healthz returns ok" \
    'curl -sf --max-time 5 http://192.168.1.203:6333/healthz | grep -qi "ok\|true\|healthy"'

# CONTRACT: Qdrant collections endpoint reachable
contract "Qdrant /collections endpoint reachable" \
    'curl -sf --max-time 5 http://192.168.1.203:6333/collections | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if "result" in d else 1)"'
...
contract "Embedding service (DEV:8001) health ok" \
    'curl -sf --max-time 5 http://192.168.1.189:8001/health >/dev/null'

# CONTRACT: Reranker service responds on DEV:8003
contract "Reranker service (DEV:8003) health ok" \
    'curl -sf --max-time 5 http://192.168.1.189:8003/health >/dev/null'
...
contract "Memory /v1/ingest endpoint exists" \
    'CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 -X POST http://192.168.1.189:8720/v1/ingest -H "Content-Type: application/json" -d "{}"); [ "$CODE" != "404" ] && [ "$CODE" != "000" ]'

# CONTRACT: Memory /v1/collections endpoint returns list
contract "Memory /v1/collections returns JSON" \
    'curl -sf --max-time 5 http://192.168.1.189:8720/v1/collections | python3 -c "import sys,json; json.load(sys.stdin)"'
...
contract "Gateway /v1/chat/completions endpoint exists" \
    'CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 -X POST http://192.168.1.189:8700/v1/chat/completions -H "Content-Type: application/json" -d "{}"); [ "$CODE" != "404" ] && [ "$CODE" != "000" ]'

# CONTRACT: OpenFang health endpoint responds
contract "OpenFang API (DEV:4200) health ok" \
    'curl -sf --max-time 5 http://192.168.1.189:4200/api/health >/dev/null'

# CONTRACT: Subscription burn scheduler health responds
contract "Subscription burn scheduler (DEV:8065) health ok" \
    'curl -sf --max-time 5 http://192.168.1.189:8065/health >/dev/null'

$QUIET || echo ""
$QUIET || echo "=== Contract Test Results: ${CONTRACT_PASS} passed, ${CONTRACT_FAIL} failed out of $((CONTRACT_PASS+CONTRACT_FAIL)) checks ==="
...
export CONTRACT_PASS CONTRACT_FAIL CONTRACT_FAILURES
