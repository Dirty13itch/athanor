...
Right now, **every server connects at 1GbE** through the USW Pro 24 PoE, despite every server having onboard 5GbE and an 8-port 5GbE switch sitting unused in the rack.

| Node | Onboard Network | Current Connection | Switch |
|------|----------------|-------------------|--------|
| Node 1 | 2x Intel X550 5GbE | 1GbE | USW Pro 24 PoE |
| Node 2 | Aquantia AQC113 5GbE + Intel 2.5GbE + WiFi 7 | 1GbE | USW Pro 24 PoE |
| VAULT | Aquantia 5GbE + RTL8125 2.5GbE + Thunderbolt 4 | 1GbE | USW Pro 24 PoE |
...
|--------|-------|-------------|
| USW Pro XG 10 PoE | 8x 5GbE RJ45 + 4x 10G SFP+ | **Nothing. Powered on, no server connections.** |
| USW Pro 24 PoE | 24x 1GbE + 2x 10G SFP+ | All servers here at 1GbE |
| UDM Pro | Gateway | Routing, DHCP, firewall |
| 3x X540-T2 / clones (loose) | 6x 5GbE RJ45 total | Not installed |
...
| Workload | Size | At 1GbE (~110 MB/s) | At 5GbE (~1.1 GB/s) | At 25GbE (~3 GB/s) |
|----------|------|---------------------|----------------------|---------------------|
...
**Important caveat:** These times assume the source can feed data at full network speed. VAULT's HDD array reads at 150-250 MB/s (single drive, no striping). A 70B FP16 model on VAULT HDD takes ~9-15 min regardless of network speed. **Models must be on VAULT's NVMe cache (or local NVMe) to benefit from 5GbE.** See ADR-003 (Storage Architecture) for the full tiered storage strategy.

Models are loaded once and stay in VRAM, so this is startup latency, not sustained throughput. **5GbE makes model loading from NVMe-backed storage tolerable. Local NVMe makes it fast.** 1GbE is painful for large models.
...
But if you want tensor parallelism across nodes (maximum performance), you need InfiniBand or RoCE. This is where a direct high-speed link between Node 1 and Node 2 would unlock capability that 5GbE alone can't.
...
Plex streaming from VAULT. 4K HDR Blu-ray remux = ~80-100 Mbps. **1GbE handles this easily.** 5GbE is irrelevant for streaming.
...
- **Servers** move to the USW Pro XG 10 PoE at 5GbE. Separate switch, separate cables.
- **UDM Pro routes between them.** When a TV streams from Plex on VAULT, traffic routes: VAULT (5GbE) → XG switch → SFP+ uplink → Pro 24 (1GbE) → TV. The TV's 1GbE port is the bottleneck, which is fine — 4K HDR remux is ~100 Mbps.
...
**Plenty of room.** Can add InfiniBand, additional 5GbE, or any other NIC.
...
- 100GbE (ConnectX-5) — would be bottlenecked to ~64 Gbps by the x4 slot, but still faster than 5GbE

### VAULT (TRX50 AERO D)

Multiple PCIe slots. LSI SAS3224 occupies one. Others available. But VAULT's high-speed connection goes through its onboard Aquantia 5GbE to the switch. Adding a faster NIC to VAULT would require the switch to also be faster, or a direct link.

### DEV

1GbE + WiFi 6. Could add a loose X540-T2 for 5GbE if desired. Low priority — DEV accesses the system via browser and SSH, not bulk data transfer.
...
### Tier 1: 5GbE Switched — Use What's Already There

Move server connections to the USW Pro XG 10 PoE. All three servers have onboard 5GbE. Just need Cat6A cables (or existing cables if they're Cat6A).
...
[USW Pro 24 PoE] ── 1GbE: IoT, APs, Lutron, management
    │ (SFP+ uplink)
[USW Pro XG 10 PoE] ── 5GbE: Node 1, Node 2, VAULT
```
...
**Multi-node inference:** Pipeline parallelism works. Tensor parallelism limited by 5GbE latency.
...
| Pipeline parallelism (vLLM) | ~1-5 Gbps peak (sequential layer-to-layer) | **Easily** — even 5GbE handles this |
...
**Not recommended yet.** 5GbE is adequate for model loading (2 min for 70B FP16). Optimize storage placement first (ADR-003) before throwing bandwidth at the problem.
...
```
  ...
  [USW Pro 24 PoE]    [USW Pro XG 10 PoE]
   1GbE management     5GbE data plane
   │  │  │  │  │       │     │     │
...
| Data | USW Pro XG 10 PoE | 5GbE | NFS storage, service APIs, model loading, dashboard |
...
| 10 | Data/Storage | 10.0.10.0/24 | Node 1, Node 2, VAULT (5GbE interfaces) |
...
| 1x X540-T2 (loose, already owned) in DEV | $0 | 5GbE for Shaun's workstation. Faster file transfers, SSH, dashboard responsiveness. |
| Cat6A cables (3-4x) | ~$20-40 | Connect all servers to the 5GbE switch that's sitting idle. |
...
| More 5GbE NICs | Every server already has onboard 5GbE. The loose X540 cards are useful for DEV but the servers don't need them. |
| WiFi upgrade for DEV | 5GbE wired (X540 card) is better and cheaper than any WiFi upgrade. |

---

## Implementation Order

1. **Now (free/cheap):** Connect Node 1, Node 2, and VAULT to USW Pro XG 10 PoE via 5GbE. Buy Cat6A cables if needed.
2. **Soon ($50-100):** Buy 2x ConnectX-3 FDR + QSFP+ cable. Install direct InfiniBand link between Node 1 and Node 2.
3. **When needed:** Install X540-T2 in DEV for 5GbE workstation access.
...
4. **VAULT NFS performance over 5GbE** — Unraid's mover process can cause NFS stale handles. Systemd automount on client side is the fix. Addressed in ADR-003.
