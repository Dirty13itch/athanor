...
- **9 Local Agents (Workforce)** — Execute domain-specific work autonomously or on command, within defined boundaries.
...
Athanor's face is a unified command center. The exact host, deployment shape, and current route inventory are implementation details owned by the current status docs and registry-backed dashboard reports; this section describes the intended product experience, not the live deployment endpoint.
...
- Chat panel (talk to any of the 9 agents)
...
- **Not a passive tool.** Athanor has operational intelligence — Claude as COO actively manages the system, directing 9 agents that do real autonomous work. But Shaun is always the owner. The system serves his vision.
...
- `docs/operations/HARDWARE-REPORT.md` — Generated hardware inventory summary
- `docs/decisions/` — 21 Architecture Decision Records
- `docs/operations/CONTINUOUS-COMPLETION-BACKLOG.md` — Current execution backlog
