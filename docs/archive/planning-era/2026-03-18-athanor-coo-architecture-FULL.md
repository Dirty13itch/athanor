...
> **Archived planning-era snapshot.**
> This document is retained for historical reasoning only. It is **not** current authority for hardware, model deployment, provider/pricing/reset truth, tool inventory, network state, or deployment/runtime ownership.
> Use the registry-backed control plane, generated truth reports, `docs/SERVICES.md`, and `STATUS.md` for live facts.
...
- `architecture/` — raw context export retained during the planning wave and later retired from the archive layer
...
**Tools DECIDED AGAINST (per the retired environment notes from this planning wave, do NOT install):**
...
**Tools BOOKMARKED for later (from the retired environment notes used during this planning wave):**
...
- claude-squad installed but **DECIDED AGAINST** per the retired environment notes from this planning wave (Agent Teams replaces it)
