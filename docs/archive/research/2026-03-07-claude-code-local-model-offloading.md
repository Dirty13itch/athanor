...
Goose is bookmarked for installation but not yet deployed. Earlier environment notes identified it as the "first candidate for an agent running entirely on Athanor hardware," but those notes were later retired from the archive because they drifted from current truth.
...
- `docs/MASTER-PLAN.md` — Stable strategic posture; live provider and routing truth now lives in the control-plane registries and generated reports
