...
Athanor has four machines (Node 1, Node 2, VAULT, DEV) sharing a home network with WiFi clients, IoT devices, Lutron lighting, and smart home infrastructure. The existing network hardware includes a UDM Pro gateway, a USW Pro XG 10 PoE (8-port 10GbE switch — currently unused by servers), and a USW Pro 24 PoE (1GbE — all servers currently connected here).

Every server has onboard 10GbE. None are using it. This is the first problem.

The second problem: Node 1 and Node 2 together have 120 GB of VRAM across 6 GPUs. Combining them for multi-node inference requires a fast, low-latency interconnect between the two machines. 10GbE is not sufficient for tensor parallelism — InfiniBand or RoCE is the established solution.
...
| **Data** | USW Pro XG 10 PoE (switched) | 10GbE | NFS storage (VAULT ↔ nodes), service APIs, model loading, dashboard traffic |
...
| Node | Data Plane (10GbE) | GPU Interconnect | Home/Mgmt (1GbE) |
|------|--------------------|------------------|-------------------|
...
| 10 | 10.0.10.0/24 | Data plane (10GbE server traffic) | Now |
...
3. **Tier 3 (if needed, deferred to ADR-003):** Faster VAULT link or local NVMe caching. 10GbE may be sufficient for storage traffic — evaluate after measuring actual model loading times.
...
- **2-minute model loads** from VAULT instead of 21 minutes (10GbE vs 1GbE for 70B FP16)
...
| Alternative | Why Not |
|-------------|---------|
| Stay at 1GbE | 21-minute model loads. Multi-node inference impractical. Wastes existing 10GbE hardware. |
| 10GbE only (no InfiniBand) | Works for storage and APIs but tensor parallelism over TCP is inefficient. Leaves 56 GB of Node 2 VRAM stranded from Node 1 for large models. |
