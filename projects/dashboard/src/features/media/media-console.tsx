          ...
          <CardContent className="space-y-4">
            {snapshot.sessions.length > 0 ? (
              snapshot.sessions.map((session) => (
                <div key={`${session.title}-${session.friendlyName}`} className="rounded-2xl border border-border/70 bg-background/20 p-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge variant="secondary">{session.state ?? "unknown"}</Badge>
            ...
            {snapshot.downloads.length > 0 ? (
              <div className="space-y-3">
                {snapshot.downloads.map((download) => (
                  <div key={download.id} className="rounded-2xl border border-border/70 bg-background/20 p-4">
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge variant="secondary">{download.source}</Badge>
                      {download.status ? <Badge variant="outline">{download.status}</Badge> : null}
                    </div>
        ...
        <Card className="border-border/70 bg-card/70">
          ...
          <CardContent className="space-y-3">
            {snapshot.launchLinks.map((link) => (
              <div key={link.id} className="flex items-center justify-between rounded-2xl border border-border/70 bg-background/20 px-4 py-3">
                <div>
            ...
            {[...snapshot.tvUpcoming, ...snapshot.movieUpcoming].length > 0 ? (
              [...snapshot.tvUpcoming, ...snapshot.movieUpcoming].map((entry) => (
                <div key={entry.id} className="rounded-2xl border border-border/70 bg-background/20 p-4">
                  <div className="flex flex-wrap items-center gap-2">
                    {entry.hasFile ? <Badge variant="secondary">Downloaded</Badge> : null}
        ...
        <Card className="border-border/70 bg-card/70">
          ...
          <CardContent className="space-y-4">
            ...
            {snapshot.watchHistory.map((entry) => (
              <div key={entry.id} className="rounded-2xl border border-border/70 bg-background/20 p-4">
                <div className="flex flex-wrap items-center gap-2">
                  {entry.watchedStatus === 1 ? <Badge variant="secondary">Completed</Badge> : null}
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between rounded-xl border border-border/60 bg-background/30 px-3 py-2 text-sm">
