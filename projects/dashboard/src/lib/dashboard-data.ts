  ...
  try {
    ...
    const liveProjects = Object.entries(result.projects).map(([id, project]): ProjectSnapshot => {
      ...
      return {
        ...
        headline: registry?.headline ?? project.description,
        status: registry?.status ?? project.status,
        kind: registry?.kind ?? "domain",
        firstClass: registry?.firstClass ?? false,
        lens: registry?.lens ?? "default",
        primaryRoute: registry?.primaryRoute ?? "/workplanner",
        externalUrl: registry?.externalUrl ?? null,
        agents: project.agents,
        needsCount: project.needs_count,
        constraints: project.constraints,
        operatorChain: registry?.operators ?? ["Claude"],
      };
    });
