...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(joinUrl(config.agentServer.url, `/v1/conventions/${conventionId}/confirm`), {
      method: "POST",
    ...
    return NextResponse.json({ error: "Failed to confirm convention" }, { status: 502 });
  }
}
