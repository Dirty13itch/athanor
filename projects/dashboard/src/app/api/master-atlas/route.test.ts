...
describe("master atlas api route", () => {
  ...
  it("returns the compiled relationship map when the generated feed is available", async () => {
    ...
    vi.mocked(pickMasterAtlasRelationshipMap).mockReturnValue({
      generated_at: "2099-04-10T00:00:00Z",
      summary: null,
  ...
  it("returns a degraded fallback when the generated feed is stale", async () => {
    vi.mocked(readGeneratedMasterAtlas).mockResolvedValue({ dashboard_summary: {} });
    vi.mocked(pickMasterAtlasRelationshipMap).mockReturnValue({
      generated_at: "2026-04-21T03:03:51.253140+00:00",
      summary: null,
      turnover_readiness: null,
      authority_surfaces: [],
      promotion_flow: {
    ...
    vi.mocked(buildFallbackMasterAtlasRelationshipMap).mockReturnValue({
      generated_at: "2026-04-21T09:45:00Z",
      available: false,
      degraded: true,
      detail: "Master atlas feed is present but stale in this dashboard runtime.",
      source: "master-atlas-fallback",
      error: "Master atlas feed is stale",
      summary: null,
      turnover_readiness: null,
      authority_surfaces: [],
      promotion_flow: {
    ...
    expect(buildFallbackMasterAtlasRelationshipMap).toHaveBeenCalledWith(
