...
const QUEEN_ARCHETYPES: Record<string, Partial<CharacterDNA>> = {
  ice_queen: {
    archetype: "ice_queen",
...
export const eoqToolDefs = [
  // 43. eoq_create_queen_route
  {
    name: "eoq_create_queen_route",
    description:
      "Create a complete queen route with DNA integration, acts, affinity tracking, and ending stubs.",
    inputSchema: z.object({
      output_dir: z
        .string()
        .describe("Directory to write the route files into"),
      queen_id: z.string().describe("Queen identifier (snake_case, e.g. 'seraphina')"),
      queen_name: z.string().describe("Display name"),
      archetype: z
        .enum([
          "ice_queen",
          "fire_dancer",
      ...
      color_hex: z.string().describe("Name color hex"),
      kink_profile: z
        .array(z.string())
        .describe("Kink/trait profile from SoulForge DNA"),
      acts: z.number().min(1).max(5).default(3).describe("Number of acts"),
      scenes_per_act: z.number().min(2).max(10).default(4),
    }),
    handler: async (args: {
      output_dir: string;
      queen_id: string;
      ...
      const charCode =
        fileHeader(`Character: ${args.queen_name}`) +
        "\n" +
        generateCharacterFromDNA(dna);
      ...
      const routeLines: string[] = [
        fileHeader(`Route: ${args.queen_name} — ${args.archetype}`),
      ...
      for (let act = 1; act <= args.acts; act++) {
        ...
        for (let scene = 1; scene <= args.scenes_per_act; scene++) {
          ...
          routeLines.push(
            `    ## DNA: corruption_vector=${dna.corruption_vector} | voice=${dna.voice_style}`
          );
  ...
  // 44. eoq_generate_corruption_arc
  {
    ...
    description:
      "Generate a 5-stage corruption progression arc for a queen, with scene stubs.",
    inputSchema: z.object({
      file_path: z.string().describe("Output .rpy file path"),
      queen_id: z.string().describe("Queen identifier"),
      queen_name: z.string().describe("Display name"),
      corruption_vector: z.string().describe("Primary corruption driver"),
      stage_descriptions: z
        .array(z.string())
        .length(5)
        .describe("Description for each of the 5 stages"),
      intensities: z
        .array(z.enum(["soft", "medium", "explicit", "extreme"]))
        .length(5)
    ...
    handler: async (args: {
      ...
      const stages: CorruptionStage[] = args.stage_descriptions.map((desc, i) => ({
        stage: i + 1,
      ...
      const code =
        fileHeader(`Corruption Arc: ${args.queen_name} — vector: ${args.corruption_vector}`) +
        "\n" +
        generateCorruptionArc(args.queen_id, stages);
  ...
  // 45. eoq_create_council_scene
  {
    ...
    description:
      "Create a Council of Queens political scene with queens present, agenda, and player choice impact.",
    inputSchema: z.object({
      file_path: z.string().describe("Output .rpy file path"),
    ...
    handler: async (args: {
      ...
      agenda: string;
      player_choices: { text: string; jump?: string; affinity_delta?: Record<string, number> }[];
      tension_level: number;
      background: string;
      music: string;
    }) => {
      const lines: string[] = [
        fileHeader(`Council Scene: ${args.label}`),
        ``,
        `label ${args.label}:`,
      ...
      // Player choice
      const menuBlock = generateChoiceMenu(
        `How do you respond to the council's deliberation?`,
        args.player_choices
      );
      lines.push(
        menuBlock
  ...
  // 46. eoq_generate_awakening_event
  {
    ...
    description:
      "Generate an Awakening event sequence — when a queen's dormant power/memory manifests.",
    inputSchema: z.object({
      file_path: z.string().describe("Output .rpy file path"),
      queen_id: z.string().describe("Queen identifier"),
      dna: z.object({
      ...
      trigger: z.string().describe("What triggers the awakening"),
      manifestation_type: z
    ...
    handler: async (args: {
      ...
      const lines: string[] = [
        fileHeader(
          `Awakening Event: ${args.dna.name} — ${args.manifestation_type}`
        ),
        ``,
      ...
      for (let i = 1; i <= args.scene_count; i++) {
        ...
        lines.push(
          `    show ${args.queen_id} dream at center with dissolve`
        );
        lines.push(
          `    ${args.queen_id} "## [${args.dna.voice_style}] Awakening dialogue ${i}"`
        );
      ...
      lines.push(
        `    $ ${args.queen_id}_awakening_complete = True`
      );
      lines.push(
        `    $ ${args.queen_id}_corruption += 1  ## Awakening increases corruption`
      );
      lines.push(`    return`);
  ...
  // 47. eoq_create_harem_wars_event
  {
    ...
    description:
      "Generate a jealousy/rivalry event between two queens with player mediation choices.",
    inputSchema: z.object({
      file_path: z.string().describe("Output .rpy file path"),
      ...
      target: z.string().describe("Queen ID of the target"),
      rivalry_type: z
        .enum(["jealousy", "dominance", "alliance_break", "sabotage"])
        .describe("Type of rivalry event"),
      player_side_choices: z
        .array(
          z.object({
            text: z.string(),
            favors: z.string().describe("Which queen this choice favors"),
            affinity_delta: z.record(z.number()).optional(),
          })
        )
    ...
    handler: async (args: {
      ...
      const lines: string[] = [
        fileHeader(
          `Harem Wars Event: ${args.instigator} vs ${args.target} — ${args.rivalry_type}`
        ),
        ``,
        `label ${args.label}:`,
      ...
      const menuBlock = generateChoiceMenu("How do you intervene?", choices);
      lines.push(
        menuBlock
          .split("\n")
          .map((l) => `    ${l}`)
      ...
      for (const choice of args.player_side_choices) {
        ...
        lines.push(
          `    ${choice.favors} "## Grateful response placeholder"`
        );
        const other =
          choice.favors === args.instigator ? args.target : args.instigator;
        lines.push(
          `    ${other} "## Resentful response placeholder"`
        );
        lines.push(`    return`);
        lines.push(``);
      }
  ...
  // 48. eoq_generate_phone_messages
  {
    ...
    description:
      "Generate obsession-level phone message sequences from a queen to the player.",
    inputSchema: z.object({
      file_path: z.string().describe("Output .rpy file path"),
      queen_id: z.string().describe("Queen identifier (sender)"),
      messages: z
        .array(
          z.object({
            timestamp_label: z.string().describe("Label suffix for this message group"),
            messages: z.array(z.string()).describe("Message texts"),
            attachment: z.string().optional().describe("Attached image/file"),
            obsession_level: z
              .number()
              .min(1)
              .max(5)
    ...
    handler: async (args: {
      ...
      const enriched: PhoneMessage[] = args.messages.map((m) => ({
        ...m,
        sender: args.queen_id,
      }));

      const header = fileHeader(`Phone Messages: ${args.queen_id}`);
      const code = generatePhoneMessages(enriched);
  ...
  // 49. eoq_create_stripper_arc
  {
    ...
    description:
      "Create a stripper arc progression for a queen — staged degradation/empowerment arc.",
    inputSchema: z.object({
      output_dir: z.string().describe("Directory for arc files"),
      queen_id: z.string().describe("Queen identifier"),
      queen_name: z.string().describe("Display name"),
      venue_name: z.string().describe("Club/venue name"),
      stages: z.number().min(3).max(7).default(5).describe("Number of arc stages"),
      arc_direction: z
    ...
    handler: async (args: {
      ...
      const lines: string[] = [
        fileHeader(
          `Stripper Arc: ${args.queen_name} @ ${args.venue_name} [${args.arc_direction}]`
        ),
        ``,
      ...
      for (let stage = 1; stage <= args.stages; stage++) {
        const intensity: IntensityLevel =
          stage <= 2 ? "soft" : stage <= 3 ? "medium" : stage <= 4 ? "explicit" : "extreme";
        ...
        lines.push(
          `    scene bg_${args.venue_name.toLowerCase().replace(/\s/g, "_")}_stage with dissolve`
        );
        ...
        lines.push(
          `    ${args.queen_id} "## [internal] Stage ${stage} monologue — ${args.arc_direction}"`
        );
        ...
        if (stage < args.stages) {
          ...
          lines.push(
            generateChoiceMenu(`Continue the arc?`, [
              {
                text: "Push further",
      ...
      const filePath = path.join(
        args.output_dir,
        `${args.queen_id}_stripper_arc.rpy`
      );
  ...
  // 50. eoq_create_ending_sequence
  {
    ...
    description:
      "Create one of the 8 EoBQ ending types as a full sequence with narration stubs, CG calls, and achievement unlock.",
    inputSchema: z.object({
      output_dir: z.string().describe("Directory for ending files"),
      queen_id: z
        .string()
        .optional()
      ...
      title: z.string().describe("Ending title"),
      conditions: z
        .array(z.string())
      ...
      music: z.string().optional().describe("Ending theme music file"),
    ...
    handler: async (args: {
      ...
      const content = fileHeader(`Ending: ${args.title} [${args.ending_type}]`) + "\n" + code;
