...
> **Status:** Reference-only operator context.
> **Startup context and current entry points live here:** `AGENTS.md`, `PROJECT.md`, `STATUS.md`, `python scripts/session_restart_brief.py --refresh`, `reports/truth-inventory/finish-scoreboard.json`, `reports/truth-inventory/runtime-packet-inbox.json`, and `docs/operations/ATHANOR-OPERATING-SYSTEM.md`.
> **Purpose:** retain durable operator context without owning mutable runtime, queue, or deployment facts.

This file is reference-only context for Athanor. It is reference context, not the live source of truth.
...
- When healthy, the dashboard and governor surfaces are the preferred control plane entry points.

Any operational claim beyond that should move into canonical docs or registries and then be removed from here.
